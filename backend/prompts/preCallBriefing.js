...
function buildPreCallBriefingMessages(doctorName, dcrHistory, mrName = '') {
  ...
  const historyText = dcrHistory.length === 0
    ...
    : dcrHistory.map((dcr, i) => {
        const samplesList = Array.isArray(dcr.samples)
          ? dcr.samples.map(s => typeof s === 'object' ? `${s.name} x${s.quantity}` : s).join(', ')
          : 'None';
        const lines = [
          `Visit ${i + 1} — Date: ${dcr.date}`,
          `  Product: ${dcr.product}`,
          `  Samples given: ${samplesList}`,
          `  Summary: ${dcr.call_summary || 'No summary provided'}`,
        ];
        if (dcr.doctor_feedback) lines.push(`  Doctor feedback: ${dcr.doctor_feedback}`);
        if (dcr.edetailing && dcr.edetailing.presented) {
          lines.push(`  E-detailing: ${dcr.edetailing.slides?.join(', ') || 'Yes'} (${dcr.edetailing.duration_mins} mins)`);
        }
        return lines.join('\n');
      }).join('\n\n');
