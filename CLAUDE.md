...
**Active phase: Phase B in progress.** Phase A complete (A.1–A.8). Phase B backend ships migration v13 (CLM + MLR with 6 new tables) + 3 new routes (`/api/content`, `/api/mlr`, `/api/content-views`) + extended `users.role` to add `medical_reviewer`, `legal_reviewer`, `regulatory_reviewer`. AI hooks (Claim Substantiation, MLR Pre-Review, Content Recommender) are next. Frontend wiring of EDetailing.tsx + ContentLibrary + MLRReviewQueue follows.
